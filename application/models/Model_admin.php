...
class Model_admin extends CI_Model {
    ...
    public function UpdateCat($id, $datos) {
        $this->db->where('Codigo', $id);
    ...
    public function Admin($usuario, $pass) {
        ...
        $query = $this->db->query("SELECT count(*) as total FROM usuarios WHERE Nombre_usuario = '".$usuario."' AND Contrasena='".$pass."' AND Borrado IS NULL AND Tipo='Admin';");
       
        $a = $query->row_array()['total'];
    ...
    public function Permiso_Admin($id) {
        $q = 'UPDATE usuarios SET Tipo="Admin" WHERE Codigo="' . $id . '";';
        $this->db->query($q);
    }
